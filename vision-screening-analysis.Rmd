...
vision <- vision %>% 
  dplyr::mutate(
    ...
    insurance = factor(x = insurance, levels = c("Uninsured", "Private", "Medicare", "Medicaid", "Goldcard", "Affordable Care Act", "Molina")) %>% ff_label("Insurance")
  )
```
...
bar <- bar %>% dplyr::mutate(
  barriers = factor(x = barriers, levels = c("Cost", "Lack of Transportation", "Lack of Knowledge", "Clinic Waiting Time", "Fear of Doctors", "Other", "None")) %>% ff_label("Barriers to Care")
)
